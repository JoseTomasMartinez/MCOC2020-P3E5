...
for k in range(np.int32(Days/dt)):
    ...
    if t> next_t:
        ...
        imshowbien(u_k)
        plt.title(titulo)
        plt.savefig("fig/caso_4_{0:04.0f}.png".format(framenum))
        framenum+=1
        next_t+= dnext_t
        plt.close(1)
...
plt.ylim(0, 20)
plt.title("Evolucion de temperatura en puntos en caso 4")
plt.legend()
plt.savefig("EvolucionT_caso_4.png")
...
plt.title("Evolucion de temperatura en puntos en caso 4 puntos")
plt.legend()
plt.savefig("EvolucionT_caso_4_puntos.png")
plt.show()